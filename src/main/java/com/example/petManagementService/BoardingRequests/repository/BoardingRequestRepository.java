...
public interface BoardingRequestRepository extends JpaRepository<BoardingRequests, UUID> {
    List<BoardingRequests> findByRequest_CareCenter_Id(UUID centerId);  // Why Request_ with the underscore: centerId, requesterUserId, and petId are not columns on boarding_requests — they're on requests. The underscore means "go into the request association, then take centerId"
